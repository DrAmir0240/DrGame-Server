...
def user_display_name(user: User) -> str:
    """
    User display name based on related entities.
    If it has main_manager => name
    If it has employee => first_name + last_name
    Otherwise: username or 'Unknown'
    """
    if not user:
        return "Unknown"
    # getattr is safer than hasattr for OneToOne access
    mm = getattr(user, "main_manager", None)
    if mm:
        return mm.name or "Unknown"
    emp = getattr(user, "employee", None)
    if emp:
        full = f"{emp.first_name or ''} {emp.last_name or ''}".strip()
        return full or (user or "Unknown")
    return "Unknown"


def employee_profile_url(user: User):
    """
    Return the profile picture URL if the user has an employee with a profile picture.
    """
    emp = getattr(user, "employee", None)
...
class LastMessageSerializer(serializers.Serializer):
    # summary output of the last message
...
class ChatRoomSerializer(serializers.ModelSerializer):
    # Note: the related_name on the Membership model for chat_room is 'memberships'.
    ...
    def get_last_message(self, obj: ChatRoom):
        # because of ordering=['-created_at'], the last message in QuerySet => first()
        ...
        return LastMessageSerializer(
            {
                ...
                "text": (msg.text or "")[:200],  # safe against None
    ...
    def get_display_name(self, obj: ChatRoom):
        """
        For pv, show the name of the "other party";
        for group/channel, return the same name.
        """
        request = self.context.get("request")
        if not request:
            # if we have no context, fallback
            return obj.name
...
class ChatRoomCreateSerializer(serializers.ModelSerializer):
    member_ids = serializers.ListField(
        ...
        write_only=True,
        help_text="List of Employee IDs to add to the chat",
    )
    ...
    def validate(self, attrs):
        ...
        # only MainManager may create (soft guard even though permissions check it too)
        if not getattr(owner, "main_manager", None):
            raise serializers.ValidationError(
                "Only Main Manager can create chat rooms."
            )
        ...
        if len(member_ids) == 0:
            raise serializers.ValidationError(
                "You must add at least one Employee to the chat."
            )

        # dedupe the list to avoid unique_together errors
        attrs["member_ids"] = list(dict.fromkeys(member_ids))

        # Rule 2: a pv has only one member
        if chat_type == ChatRoom.PV and len(attrs["member_ids"]) != 1:
            raise serializers.ValidationError(
                "Private chat (pv) must have exactly one Employee."
            )
    ...
    def create(self, validated_data):
        ...
        # the owner is always a member and admin
        Membership.objects.create(
            user=owner, chat_room=chat_room, is_admin=True, is_muted=False
        )

        # fetch the target employees (with select_related for performance)
        employees = Employee.objects.filter(id__in=member_ids).select_related("user")

        # if fewer than requested were found, we have invalid IDs
        ...
        for emp in employees:
            # members are muted by default in channels
            Membership.objects.create(
                user=emp.user, chat_room=chat_room, is_muted=is_channel
            )

        # Rule 3: at least one member other than the owner
        non_owner_count = chat_room.users.exclude(id=owner.id).count()
        if non_owner_count == 0:
            raise serializers.ValidationError(
                "Chat room must have at least one member besides the owner."
            )
...
class ChatRoomUpdateSerializer(serializers.ModelSerializer):
    member_ids = serializers.ListField(
        ...
        required=False,
        write_only=True,
        help_text="If provided, current members (except owner) are replaced with this list.",
    )
    ...
    def update(self, instance: ChatRoom, validated_data):
        ...
        # only the owner may edit members/name
        if instance.owner_id != user.id:
            raise serializers.ValidationError(
                "Only the owner can update the chat room."
            )
        ...
        if member_ids is not None:
            ...
            Membership.objects.filter(chat_room=instance).exclude(
                user=instance.owner
            ).delete()

            employees = Employee.objects.filter(id__in=member_ids).select_related(
                "user"
            )
            if employees.count() != len(member_ids):
                raise serializers.ValidationError(
            ...
            for emp in employees:
                Membership.objects.create(
                    user=emp.user, chat_room=instance, is_muted=is_channel
                )

            # check at least one member other than the owner
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    # simple input for reply: only takes an id
    ...
    def validate(self, attrs):
        ...
        - If reply_to_id is present, the target message must be in the same room.
        """
        request = self.context["request"]
        user = request.user

        room = attrs.get("room")  # comes as a PrimaryKeyRelatedField
        if not room:
        ...
        if reply_to_id:
            ...
            if reply_msg.room_id != room.id:
                raise serializers.ValidationError(
                    "reply_to message must be in the same room."
                )
            # kept for create
    ...
    def create(self, validated_data):
        """
        - Set the logged-in user as the sender.
        - If reply_to was set in validate, use it.
        """
        user = self.context["request"].user
        validated_data["sender"] = user
        # keep reply_to here if validate set it
        validated_data.pop(
            "reply_to_id", None
        )  # in case it remained in attrs for any reason
...
class MessageEditSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance: Message, validated_data):
        """
        Edit the text and set is_edited=True
        """

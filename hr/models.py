...
class Permission(models.Model):
    """
    Each record = one atomic permission
    Example: module='accounting', action='read'
    """
    ...
    def __str__(self):
        return f"{self.module}:{self.action}" + (
            f":{self.extra_flag}" if self.extra_flag else ""
        )
...
class Employee(models.Model):
    ...
    profile_picture = models.ImageField(
        null=True, blank=True, upload_to="profile_pictures/hr/"
    )
...
class EmploymentResume(models.Model):
    ...
    resume_file = models.FileField(
        upload_to="hire/resume_files/", null=True, blank=True
    )
...
class EmployeeArrival(models.Model):
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name="arrivals"
    )
...
class EmployeeOvertime(models.Model):
    employee = models.ForeignKey(
        Employee, on_delete=models.PROTECT, related_name="overtimes"
    )
